...
async function seed() {
  ...
  try {
    ...
    for (const p of presentations) {
      await client.query(
        'INSERT INTO presentations (title, image_url, description, excerpt, visible) VALUES ($1, $2, $3, $4, $5)',
        [p.title, p.image_url, p.description, p.excerpt, p.visible]
      );
    }
    ...
    for (const e of events) {
      await client.query(
        'INSERT INTO events (title, date, time, location, description, link, image_url, event_date) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',
        [e.title, e.date, e.time, e.location, e.description, e.link, e.image_url, e.event_date]
      );
    }
